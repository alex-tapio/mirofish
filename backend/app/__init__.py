...
def create_app(config_class=Config):
    ...
    # JSON encoding: allow unicode characters through without escaping
    if hasattr(app, 'json') and hasattr(app.json, 'ensure_ascii'):
        app.json.ensure_ascii = False

    logger = setup_logger('mirofish')

    # Only log startup in reloader subprocess (avoid double-logging in debug mode)
    ...
    # Settings endpoint — allows frontend to pass user API keys
    @app.route('/api/settings/test', methods=['POST'])
    def test_settings():
        """Test that the provided API key and model work"""
        ...
        if not api_key:
            return {'success': False, 'error': 'API key is required'}, 400
        ...
        try:
            ...
            return {'success': True, 'message': f'Connection successful. Model responded: {response}'}
        except Exception as e:
